#!/usr/bin/env python3
"""
Convert lines 1-3 from linkdown_events.txt to charles_event.json format
...
# Read first 3lines from linkdown_events.txt
print("Reading first 3 lines from linkdown_events.txt...")
events = []
...
# Save to JSON file
output_file = 'linkdown_batch_1-3.json'
    ...
    try:
        ...
        if http_code == '200':
            ...
            try:
                ...
                print(f"Event {i}: ✅ Success (UID: {uid})")
            except:
                print(f"Event {i}: ✅ Success")
        else:
            fail_count += 1
            print(f"Event {i}: ❌ Failed (HTTP {http_code})")
    except Exception as e:
        fail_count += 1
